...
Now, our graph is prepared. We used Dijkstra shortest path algorithm fo calculate the shortest path between starting point to hospital to patient and so on. This algorithm runs only 4 times. From starting point to first patient, first patient to hospital, hospital to second patient and second patient to the hospital. For making a step from one node to other node we are using functions like posdetect() which detects the current position of the bot using aruco marker code and returns it’s co-ordinates. Using these co-ordinates, we got to know the direction in which bot is pointing i.e., it’s position vector. We have another vector directvector which represents the direction in which the bot has to move and which changes from node to node following Dijkstra shortest path.
